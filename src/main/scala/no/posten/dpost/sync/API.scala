...
case class Document(val subject: String, val creatorName: String, val link: List[Link]) {
  def links = link
}
...
trait API {
  ...
  def download(link: Link, toFile: File) = {
    ...
    for {
      ...
      jj <- Control.trapAndFinally(http(url(link.uri) <:< headers >>> toStream))(toStream.close)
    } yield Unit
  }
